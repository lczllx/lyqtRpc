...
namespace lcz_rpc
{
    namespace metrics
    {
        ...
        public:
            ...
            static void onRequestStart(std::string_view method)
            {
                Labels ls = {{"method", std::string(method)}};
                METRICS_GAUGE("rpc_concurrency", "In-flight requests", ls).inc();
            }
            static void onRequestEnd(std::string_view method, int rcode, double lat_us)
            {
                Labels ls = {{"method", std::string(method)}};
                ...
                if (rcode != 0)
                {
                    ...
                    Labels els = {{"method", std::string(method)}, {"code", reason}};
                    METRICS_COUNTER("rpc_errors_total", "RPC handler errors", els).inc();
                }
            }

            // 客户端侧: send() 时记录
            static void onClientSend(std::string_view method)
            {
                Labels ls = {{"method", std::string(method)}};
            ...
            static void onClientRecv(std::string_view method, double lat_us, std::string_view error_code)
            {
                Labels ls = {{"method", std::string(method)}};
                ...
                if (!error_code.empty())
                {
                    Labels els = {{"method", std::string(method)}, {"code", std::string(error_code)}};
                ...
                else
                {
                    ...
                    {
                        Labels els = {{"method", std::string(method)}, {"code", ec}};
            ...
            static void onCircuitState(std::string_view method, std::string_view host, int state)
            {
                Labels ls = {{"method", std::string(method)}, {"host", std::string(host)}};
                METRICS_GAUGE("circuit_breaker_state", "Circuit breaker state", ls).set(state);
            }

            // 令牌桶余量
            static void onTokenBucket(std::string_view service, double available)
            {
                Labels ls = {{"service", std::string(service)}};
                METRICS_GAUGE("token_bucket_available", "Token bucket available tokens", ls).set(available);
            }

            // 限流拒绝计数（对标 brpc 被拒绝请求数）
            static void onRateLimited(std::string_view service)
            {
                Labels ls = {{"service", std::string(service)}};
            ...
            static void onRegistryHeartbeat(std::string_view method, bool success)
            {
                Labels ls = {{"method", std::string(method)}};
                METRICS_COUNTER("registry_heartbeats_total", "Registry heartbeats", ls).inc();
                if (!success)
                {
                    Labels els = {{"method", std::string(method)}, {"code", "failed"}};
