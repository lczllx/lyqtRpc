    ...
    namespace server
    {
        ...
        public:
            ...
            virtual void registerInstance(const BaseConnection::ptr &conn,
                const HostInfo &host, std::string_view method, int load) = 0;
            // 服务发现：返回指定方法的全部主机地址
            virtual std::vector<HostInfo> methodHost(std::string_view method) = 0;
            // 服务发现（带负载）：返回指定方法的全部主机 + 负载
            virtual std::vector<HostDetail> methodHostDetails(std::string_view method) = 0;
            // 负载上报：更新指定 method+host 的负载值
            virtual bool reportLoad(std::string_view method, const HostInfo &host, int load) = 0;
            // 心跳：刷新指定 method+host 的最后心跳时间
            virtual bool heartbeat(std::string_view method, const HostInfo &host) = 0;
