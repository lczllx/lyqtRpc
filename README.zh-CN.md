...
SHM 路径吞吐是 brpc TCP 的 2.0 倍（16B 单线程）、3.0 倍（64KB 单线程）、2.7 倍（4 线程）。TCP 路径单线程 16B QPS 约为 brpc 的 72%，64KB 降至 52%，根因是 bthread 协程、IOBuf 零拷贝链、baidu_std 多路复用等架构差异。
...
- TCP 路径已按 host 复用长连接（连接池），但同一 host 仍是单连接，多线程打同一 host 时 `send()` 持锁排队；协议多路复用（stream_id）待做
