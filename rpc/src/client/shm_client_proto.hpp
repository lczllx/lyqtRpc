...
public:
    ...
    void connect() override {
        ...
        if (conn_fd < 0) { LCZ_ERROR("[ShmClientProto] socket failed"); return; }
        ...
        if (!ShmChannel::handshake_client(conn_fd, req_fd, resp_fd, _shm_name)) {
            LCZ_ERROR("[ShmClientProto] handshake failed"); close(conn_fd); return;
        }
        close(conn_fd);
        if (!_channel.open(_shm_name)) { LCZ_ERROR("[ShmClientProto] open failed"); return; }
    ...
    bool send(const BaseMessage::ptr& msg) override {
        ...
        if (buf && contig >= req->byteSize()) {
            ...
            if (!_channel.write_request(body, MsgType::REQ_RPC_PROTO)) return false;
            _channel.notify_req();
            return true;
        }
...
private:
    void responseLoop() {
        ...
        ev.events = EPOLLIN; ev.data.fd = _channel.resp_notify_fd();
    ...
    ShmChannel _channel; ShmConnection::ptr _conn;
