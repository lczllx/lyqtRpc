...
public:
    ...
    void start() override {
        ...
        if (listen_fd < 0) { LCZ_ERROR("[ShmServerProto] socket failed"); return; }
        ...
        if (bind(listen_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0
            ...
            LCZ_ERROR("[ShmServerProto] bind/listen failed"); close(listen_fd); return;
        }
        ...
        while (_running) {
            ...
            auto* ch_raw = ch.get();
            workerLoop->runInLoop([ch_raw]() { ch_raw->enableReading(); });
    ...
    ~ShmServerProto() {
        ...
        std::lock_guard<std::mutex> lk(_mtx);
        _channels.clear();
        for (auto& e : _clients) e->channel.destroy();
        for (auto& w : _workers) w.loop->quit();
    }

private:
    struct ClientEntry { ShmChannel channel; ShmConnection::ptr conn; };
    ...
    struct Worker { std::unique_ptr<muduo::net::EventLoopThread> thread; muduo::net::EventLoop* loop; };
