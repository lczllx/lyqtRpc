...
namespace lcz
{
    ...
    public:
        ...
        virtual void log(std::string_view data, size_t len) = 0; // 日志落地接口
    ...
    public:
        ...
        StdoutSink() = default;
        virtual void log(std::string_view data, size_t len) override
        {
    ...
    public:
        ...
        virtual void log(std::string_view data, size_t len) override
        {
            _ofs.write(data.data(), len); // 写入这个流中
            assert(_ofs.good());
            //  //立即刷新缓冲区到操作系统
            //  _ofs.flush();
        }
    ...
    public:
        // 构造时传入文件名，并打开文件，将操作句柄管理起来
        RollfileSink(std::string basename, size_t max_size)
            : _basename(std::move(basename)), _max_size(max_size), _cur_size(0)
        {
            std::string pathname = craetefilename();
            lcz::utility::File::createDirectory(utility::File::path(pathname));
            _ofs.open(pathname.data(), std::ios::binary | std::ios::app);
            assert(_ofs.is_open());
        }
        virtual void log(std::string_view data, size_t len) override
        {
    ...
    private:
        std::string craetefilename()
        {
            ...
            localtime_r(&t, &tt);
            char buf[160];
            snprintf(buf, sizeof(buf), "%s%04d%02d%02d_%02d%02d%02d_%d.log",
                     _basename.c_str(),
                     tt.tm_year + 1900, tt.tm_mon + 1, tt.tm_mday,
                     tt.tm_hour, tt.tm_min, tt.tm_sec, seq++);
    ...
    public:
        // 构造时传入文件名，并打开文件，将操作句柄管理起来
        RollbytimefileSink(std::string basename, time_t roll_interval)
            : _basename(std::move(basename)), _roll_interval(roll_interval), _last_roll_time(0)
        {
            std::string pathname = craetefilename();
            lcz::utility::File::createDirectory(utility::File::path(pathname));
            _ofs.open(pathname.data(), std::ios::binary | std::ios::app);
