...
#define INCREMENT_BUF_SIZE (1 * 1024 * 1024)    // 超过阈值后每次扩容 1MB
    ...
    public:
        ...
        void push(std::string_view data, size_t len)
        {
            ...
            getenoughsize(len);
            // 将数据拷贝进缓冲区
            std::copy(data.data(), data.data() + len, _buf.begin() + _write_pos);
        ...
        // 返回可读数据起始地址
        std::string_view begin() { return std::string_view(&_buf[_read_pos], abletoreadlen()); }
        ...
        // 返回移动读写位置
        std::string_view readpos() { return std::string_view(&_buf[_read_pos], abletoreadlen()); }
        std::string_view writepos() { return std::string_view(&_buf[_write_pos], abletowritelen()); }
